...
fastaFile = sys.argv[
    1
]  # from bash script (it's always just fasta.fa here - don't actually need to use sys.argv but why not)
...
for entry in sequences:
    ...
    if (
        newlineEntry != -1
    ):  # if '/n' isn't found find() returns -1. so if != -1 then it has found a '/n'
...
sortedGCPercent = sorted(
    sequencePercentCGDict.items(), key=lambda x: x[1], reverse=True
)

for sequence, cg in sortedGCPercent:
